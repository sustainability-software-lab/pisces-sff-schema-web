...
class FakeConda:
    ...
    def __init__(self, existing=(), fail_create=False, root=None):
        self.existing = list(existing)
        self.fail_create = fail_create
        self.root = root or ("C:\\envs" if os.name == "nt" else "/tmp/conda-envs")
        self.calls = []
        self.kwargs = []
    ...
    def __call__(self, cmd, **kwargs):
        ...
        if cmd[1:4] == ["env", "list", "--json"]:
            ...
            payload = json.dumps(
                {"envs": [self.root + separator + name for name in self.existing]}
            )
